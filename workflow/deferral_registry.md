...
| Profile `rmx context` at 19,829 docs / 406,885 concepts | `src/refmatrix/context.py`, `recall.py`, `reranker.py` | `scaffold-no-caller` (no profiling harness exists for this scale) | Measured 2026-09-15: `context` 62.9 s, `scan-prompt` 75.9 s per query, daemon-warm. Cause unattributed across BM25 fan-out / PPR walk / cross-encoder rerank. Graduates when one call is profiled and the 63 s is attributed. See `workflow/review-output/longmemeval-latency.md`. | plan 7 |
