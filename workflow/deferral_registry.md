...
| Attribute the ~8.5 s CLI-vs-`build_context` gap on a large store | `src/refmatrix/cli.py` `cli_entry`, `Store._connect` | `scaffold-no-caller` (no startup profiler) | PROFILED 2026-09-16: `build_context` warm 0.45 s, full daemonless CLI 9.0 s, only 2.8 s of it CPU. ~6 s is wait, unattributed. This SUPERSEDES the retracted 62.9 s row — see `workflow/review-output/longmemeval-latency.md`. | plan 7 |
