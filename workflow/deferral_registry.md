...
| Attribute the ~8.5 s CLI-vs-`build_context` gap on a large store | `src/refmatrix/cli.py` `cli_entry`, `Store._connect` | `scaffold-no-caller` (no startup profiler) | PROFILED 2026-09-16: `build_context` warm 0.45 s, full daemonless CLI 9.0 s, only 2.8 s of it CPU. ~6 s is wait, unattributed. This SUPERSEDES the retracted 62.9 s row — see `workflow/measurements/longmemeval-latency.md`. | plan 7 |
...
| Rerank windows the doc instead of taking its head | `src/refmatrix/reranker.py` `MAX_DOC_CHARS`; windower exists at `src/refmatrix/kwic.py` | `inert-sentinel` (the bound is applied and looks deliberate; the strategy is the defect) | bug-032. 36.9% of LongMemEval answer-bearing turns are past char 2048. Graduates when rerank scores a query-anchored window; re-measure `single-session-preference` (0.200) after. Touches the always-on hook path, so it is a ranking change and wants its own plan. | bug-032 |
