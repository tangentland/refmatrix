...
| Q4 {#q4} | (task 4.4) Operator step for orderly? | None: the supervised start adopts the unsupervised daemon on its own; the deploy at 21:54 did it (pid 39128 → 27922, launchd `runs` static). | 2026-09-14 |
| Q12 | (r3 #b-1, amends Q11) launchd is a supervisor too | ONE stop grace: `launchctl.EXIT_TIMEOUT_S` (45) is the plist's `ExitTimeOut`, the hub's `DEFAULT_KILL_GRACE_S` and the daemon's `DEFAULT_STOP_GRACE_S` (`RMX_STOP_GRACE_S` overrides). `rmx daemon restart` under launchd: pause the hub watchdog for the root, `graceful_stop` with the grace, `kickstart` WITHOUT `-k` when the daemon stopped (KeepAlive respawns it), `-k` only when it ignored the signal — said, because that is SIGTERM + a SIGKILL at ExitTimeOut. `_verify_relaunch` SIGKILLs a clinging predecessor only when its heartbeat is stale; a fresh one is working and the command fails loud. The standalone path uses the same grace and refuses to spawn beside a predecessor that is still alive. A forced plist reinstall (the drift fixer) and a forced hub reinstall stop the process gracefully BEFORE launchd's bootout, and the hub's plist carries `ExitTimeOut` too. Acceptance: zero `Sending SIGKILL` lines on `com.refmatrix.daemon.*` in launchd's log after the next `relaunch-fleet`. LIVE 2026-09-15 05:27–05:40 (`deploy-r6-r4.log`): two full `relaunch-fleet` runs (the first re-rendered all 8 plists; every label now `exit timeout = 45`, the hub too) — **0 `Sending SIGKILL` lines on `com.refmatrix.*`** in launchd's log since the deploy started (24 in the 4 h before). | 2026-09-15 |
| Q13 | (r3 #b-2, amends Q11) The shutdown produces its own signal | `shutdown.started` is written as the first act of serve_forever's finally and removed at boot; `Watchdog._await_shutdown` waits on THAT marker (younger than `daemon.SHUTDOWN_BUDGET_S`, 60 s = the drain's derivation) with the pid alive, bounded by what is left of the budget. The heartbeat is not read there: the shutdown stops it first, so its age only grows and the r2 guard could never fire (pattern guard-cannot-fire). The caller's precondition is written beside the callee's branch. Proven through `_check` on a real draining process. | 2026-09-15 |
| Q14 | (r3 #s-3) A relaunch is not a wedge | The watchdog's no-process branch kicks on the SECOND consecutive no-process tick (`noproc_counts`; launchd's KeepAlive covers the interval); `rmx daemon restart` under launchd pauses the hub watchdog for the root (`pause` RPC, 2 × grace + 60 s) and resumes after — a hub that is down is said, never fatal. | 2026-09-15 |
| Q15 | (r3 #s-4) The plist runs the caller's binary, never a dev tree | `render_plist` / `check` / `reinstall` / `install` take `rmx=`; `relaunch-fleet` passes the binary it resolved; `launchctl.binary_identity` asks the binary itself (`rmx version -v --json`) and `install` refuses a dev-tree binary before anything is booted out or written (`allow_dev` overrides); `check` names it as drift that must not be "fixed". | 2026-09-15 |
| Q16 | (r3 #m-5/#m-6) The kept pid, the said beat | `graceful_stop` records the pid it waited on (`report["pid"]`: pid file or ping-discovered) and `stop_daemon` escalates on it — a stale pid file no longer returns "stopped" on a running daemon; `Daemon._heartbeat_touch` logs a failed touch once per errno. | 2026-09-15 |
