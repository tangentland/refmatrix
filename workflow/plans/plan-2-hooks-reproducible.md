...
| Q10 | (r8 live re-measure, amends Q7) The partition probe reads the replica first | `verbs.memory_partition` and the CLI's `_legacy_memory_partition_exists` answer from the lock-free replica (`search.cached_replica` / `_reader_store()`) and ask the daemon's `partition_list` (which takes the writer lock daemon-side) only when there is no replica yet; a bootstrap-window root raises a typed FileNotFoundError and caches nothing. The recent path spends at most `RECALL_DAEMON_SLICE_S` (1.5 s) on the daemon when a replica exists. Live 2026-09-15 03:36: a watcher flush of two edited files held the store lock and every hook probe timed out into `[]` — the operator's own edits made the hooks blind. The held-writer simulation now HOLDS its catalog open (a seeded-but-released `catalog.duckdb` read as a replica, a state no live daemon permits). | 2026-09-15 |
| Q11 | (r9 live re-measure, amends Q7) The rerank pool is capped for the hook | Measured on the shared worker 2026-09-15 with the hook's real pool (10 memory docs as extracted: 28.7k chars, max 4.2k): 4.8 s — the whole 5 s hook — and the worker kept scoring after the client left, so the NEXT hook's 1 s probe queued behind it and read "shared worker unavailable" (the live "0 of N reranked" alternation). The cross-encoder truncates at 512 tokens, so 1024 chars/doc still cost 5.1 s; 768 → 3.4 s; 512 → 2–3 s. `collect_rerank_docs(doc_chars=)` caps each doc; the hook's replica leg passes `RERANK_DOC_CHARS` (700: a memory's title + lead). Open (todo G13): the worker should report its seconds-per-doc so a rerank that cannot fit is skipped up front rather than timed out, and the two per-prompt hooks (`scan-prompt`, `memory recall --stdin-json`) race one serialized worker. | 2026-09-15 |
| Q9 | (r7 #b-1/#m-3, amends Q7) Per-call timeouts are per call | `SharedWorkerClient.call(timeout=)` restores the client's own timeout afterwards — the 1 s `info` probe had stayed on the socket and shortened the `rerank` that followed to 1 s (a warm worker scores 20 docs in ~1 s: 0 of 12 live hook runs reranked); the same leak left the daemon's 30 s client at its 45 s probe. A budgeted `hub.global_call` (`retries=0`) never runs `ensure_global_daemon` (bare pings + a 30 s spawn wait outside the deadline): absent → fails fast, "global rows omitted". | 2026-09-15 |
