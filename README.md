...
The hook commands themselves, per event:

<!-- hooks-table:start -->
Generated by `scripts/gen-cli-tree.py` from `hooks._claude_hook_block` (default flags, enforcement entries omitted); `rmx install-hooks --check` proves a project's installed file equals this generator's render.

| Event | Matcher | Command |
|---|---|---|
| `PostToolUse` | `Edit\|Write\|MultiEdit\|NotebookEdit` | `( python3 -c "import json,sys,subprocess;d=json.load(sys.stdin);ti=d.get('tool_input') or {};paths=set();fp=t…` |
| `PostToolUse` | `Edit\|Write\|MultiEdit\|NotebookEdit\|Read\|Bash\|Grep\|Glob` | `rmx focus hook --event tool 2>/dev/null \|\| true` |
| `Stop` | — | `( rmx sync --flush-queue --async >/dev/null 2>&1 \|\| true ) & disown` |
| `Stop` | — | `rmx focus hook --event say 2>/dev/null \|\| true` |
| `Stop` | — | `rmx focus summarize --promote --timeout 5` |
| `SubagentStop` | — | `( rmx sync --flush-queue --async >/dev/null 2>&1 \|\| true ) & disown` |
| `SessionStart` | `startup\|resume` | `( rmx sync --flush-queue --async >/dev/null 2>&1 \|\| true ; rmx primer --out '<project>/.refmatrix/PRIMER.md…` |
| `SessionStart` | `startup\|resume\|clear` | `rmx memory recall --session-start --k 10 --scope both --json --timeout 10` |
| `SessionStart` | `startup\|resume\|clear` | `rmx ingest-gmd --as-memory --detach '~/.claude/projects/<slug>/memory'` |
| `SessionStart` | `resume` | `rmx focus context --top 15` |
| `UserPromptSubmit` | — | `rmx scan-prompt --composite-every 3 --max-tokens 2000 --timeout 5 2>/dev/null \|\| true ; rmx curator status…` |
| `UserPromptSubmit` | — | `rmx focus hook --event input 2>/dev/null \|\| true` |
| `UserPromptSubmit` | — | `rmx memory recall --stdin-json --k 5 --scope both --json --timeout 5` |
| `PreToolUse` | `Edit\|Write\|MultiEdit\|NotebookEdit\|Read\|Bash\|Grep\|Glob` | `rmx focus hook --event tool-pre 2>/dev/null \|\| true` |
| `PreCompact` | — | `rmx focus summarize --promote --timeout 30` |
| `PreCompact` | — | `rmx memory recall --recent --since 1h --k 20 --scope both --json --timeout 30` |
| `PreCompact` | — | `rmx save-state --no-promote --no-sync -m "auto: pre-compact checkpoint"` |
<!-- hooks-table:end -->
