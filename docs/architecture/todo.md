...
| G14 | Open | a daemon that fails the shared-worker probe at boot (the hub down or cold) goes private for its whole life: after the hub returns the fleet runs N private model processes beside the shared pair and CPU oversubscription makes the shared rerank 5–20× slower (bug-024, 2026-09-15 05:37: 16 workers, 10 docs in 16–25 s). Proposed: `_model_client` re-probes the shared socket every RMX_SHARED_REPROBE_S (300) while private and switches when it answers, closing the private worker; the hub's `queues`/`status` names daemons running private workers | plan-4 round 4 (2026-09-15), `deploy-r6-r4.log` |
