...
<!-- cli-tree:start -->
Generated by `scripts/gen-cli-tree.py` from `refmatrix.cli.main` (201 visible commands); `tests/test_docs_generated.py` fails when this drifts from the code.
...
│   ├── brief                      What the corpus knows solidly, and where it is thin.
...
│   └── sync-disk                  Alias of `rmx ingest-gmd --as-memory <dir>` — the memory bridge.
